...
icon: "code-branch"
description: "Crie ferramentas personalizadas via API que seu assistente de IA pode acionar durante chamadas, integrando CRMs, sistemas internos e webhooks."
---
...
Testes Facilitados
<Card title="Testes Automáticos" icon="vial">
  Clique em **Testar Ferramenta** para executar um teste com dados fictícios:
  Parâmetros string: "test", Parâmetros numéricos: 1, Parâmetros booleanos: true.
...
<AccordionGroup>
  <Accordion title="Consulta de Pedido" icon="magnifying-glass">
    Name: check\_order, Endpoint: https://api.sualoja.com/pedidos/\{order\_number}, Parameters: order\_number (string) – "Referência do pedido (formato: ORD-XXXXX)".
  ...
  <Accordion title="Disponibilidade de Agenda" icon="calendar">
    Name: check\_slots, Endpoint: https://api.calendario.com/disponibilidade, Parameters: service (string) – "Tipo de serviço (corte, massagem, consulta)"; date (string) – "Data desejada (dd/mm/aaaa)".
  ...
  <Accordion title="Validação de Cliente" icon="shield-check">
    Name: verify\_customer, Endpoint: https://api.crm.com/verify, Parameters: phone (string) – "Número de telefone com 10 dígitos"; email (string) – "Endereço de e-mail para validação".
...
<Info>
  A IA precisa de instruções claras no prompt de sistema para utilizar corretamente as ferramentas personalizadas.
</Info>
...
<Tip>
  Teste suas ferramentas com diferentes fluxos de conversa para garantir que a IA lide bem com todos os cenários. Comece com chamadas de teste simples antes de colocar em produção.
</Tip>
